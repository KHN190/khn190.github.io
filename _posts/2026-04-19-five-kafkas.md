---
layout: post
title:  "Five Kafkas in a Zoo (2026)"
date:   2026-04-19 00:00:00
categories: interactives
preview: /assets/img/covers/fk.jpeg
---

![Recruitment poster](/assets/img/machine/fk_poster.jpeg)

On April 19, 2025, six of us — five Kafkas and a child — went to a Zoo in Chengdu. We read the day's news under a bamboo grove, then wandered. Afterwards we uploaded photographs, fragments and diaries to [are.na](https://www.are.na/3vrklabqelm/caesirtynw), and from those traces I edited a stage script in the form of a theatre dialogue.

The recording below is a reading of that script. Each participant is voiced by a deceased artist or writer, all of whom — in this Chinese rendition — speak with a strong countryside accent. Some sound from Northeast China, and some sound like a Cantonese local.

<audio controls preload="metadata" style="width:100%; margin: 1.5em 0;">
  <source src="{{ '/assets/audio/five_kafkas.mp3' | prepend: site.baseurl }}" type="audio/mpeg">
  Your browser does not support the audio element.
</audio>

AI naturally presents an alienated feel — a kind of Kafka-world quality.

There is another thread in it though. In the diaries we collectively submitted, a child shrieks at the giraffe — *I'll pull off your legs* — and immerses himself in it with pleasure. 

Many of us work hard to prove they are real humans. People try to prove it in their relationships, in their families, to their browsers. But it should be an undoubtable question. The doubt itself, is a reflection of what a person themself is.

I felt AI's texture is what presents it into the visible. Although, it remains unclear how to improve the work to be more articulated.

**Cast of voices**

- rui-ya — Simone de Beauvoir
- Lia — Frida Kahlo
- Lingxiao — Nam June Paik
- 苗子 — Sylvia Plath
- breobyhnaug — Susan Sontag
- 小马 — Isamu Noguchi
- _default / narrator_ — Virginia Woolf

<div class="row">
  <div class="column column-33">
    <img src="/assets/img/machine/fk_gf01.jpeg" alt="Giraffe in a concrete room">
  </div>
  <div class="column column-33">
    <img src="/assets/img/machine/fk_gf02.jpeg" alt="Empty enclosure with a painted sky">
  </div>
  <div class="column column-33">
    <img src="/assets/img/machine/fk_gf03.jpeg" alt="Giraffe licking the glass">
  </div>
</div>

![Drawing — girl, baboon, table, sprouting tree](/assets/img/machine/fk_tb.jpeg)

**Stage script**

The full bilingual text — diary entries, a Wallace Stevens poem, a black swan poem, fragments of Kafka's diaries, and the recruitment notice — is available as a plain text file:

- [Download the script (.txt)]({{ '/assets/downloads/five_kafkas_stage.txt' | prepend: site.baseurl }})

---

Participants: rui-ya, Lia, Lingxiao, 苗子, breobyhnaug, 小马, and 超酱 (the kid).

License: [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/deed.sv)

Date: April, 2026
